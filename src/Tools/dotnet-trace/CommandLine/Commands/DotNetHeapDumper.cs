...
public class DotNetHeapDumper
{
    ...
    /// dump. returns true if successful. 
    /// </summary>
    public static bool DumpEventPipe(int processID, MemoryGraph memoryGraph, TextWriter log, DotNetHeapInfo dotNetInfo = null)
    {
        var sw = Stopwatch.StartNew();
        var dumper = new DotNetHeapDumpGraphReader(log);
        dumper.DotNetHeapInfo = dotNetInfo;
        ...
        try
        {
            ...
            // {
            //     var GCProvider = new Provider("Microsoft-Windows-DotNETRuntime");
            //     typeFlushSession = new EventPipeSession(processID, new List<Provider> { GCProvider });
            //     otherListening = true;
            //     log.WriteLine("{0,5:n1}s: Flushing the type table", sw.Elapsed.TotalSeconds);
            //     typeFlushSession.Source.Clr.GCStop += (data) => fDone = true;
            //     typeFlushSession.Source.Process();
            //     log.WriteLine("{0,5:n1}s: Done flushing the type table", sw.Elapsed.TotalSeconds);
            // });
            // while (!otherListening)
            //     typeTableFlushTask.Wait(1);
            ...
            readerTask = Task.Factory.StartNew(delegate
            {
                // string sessionName = "PerfViewGCHeapSession";
                // session = new TraceEventSession(sessionName, null);
                var GCProvider = new Provider(
                        "Microsoft-Windows-DotNETRuntime",
                        // (ulong)(ClrTraceEventParser.Keywords.GCHeapSnapshot | ClrTraceEventParser.Keywords.GC),
                        // (ulong)(ClrTraceEventParser.Keywords.GCHeapDump | ClrTraceEventParser.Keywords.GCHeapSnapshot | ClrTraceEventParser.Keywords.GC | ClrTraceEventParser.Keywords.Type | ClrTraceEventParser.Keywords.GCHeapAndTypeNames),
                        (ulong)(0x101980099),
                        System.Diagnostics.Tracing.EventLevel.Verbose
                    );
                // var GCProvider = new Provider("Microsoft-Windows-DotNETRuntime");
                session = new EventPipeSession(processID, new List<Provider> { GCProvider });
                int gcNum = -1;
                // session.BufferSizeMB = 1024;         // Events come pretty fast, so make the buffer bigger. 
            ...
            for (; ; )
            {
                ...
                if (sw.Elapsed.TotalSeconds > 20)       // Time out after 100 seconds. 
                {
    ...
    public class EventPipeSession
    {
        ...
        public EventPipeSession(int pid, List<Provider> providers)
        {
            _pid = pid;
            ...
            _eventPipeStream = EventPipeClient.CollectTracing(pid, config, out _sessionId);
